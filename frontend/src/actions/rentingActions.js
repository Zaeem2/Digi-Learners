...
export const createRentingReview =
  (rentingId, review) => async (dispatch, getState) => {
    try {
      dispatch({
...
export const addReply = (id, text) => async (dispatch, getState) => {
  ...
  const { data } = await axios.post(
    `/api/rentings/${id}/reply`,
    { text },
    config
  )

  dispatch({
    type: RENTING_ADD_REPLY,
    payload: data,
  })
}
...
export const toggleLike = (id, rid) => async (dispatch, getState) => {
  ...
  const { data } = await axios.post(
    `/api/rentings/${id}/reply/${rid}/like`,
    {},
    config
  )

  dispatch({
    type: RENTING_TOGGLE_LIKE,
    payload: data,
  })
}
